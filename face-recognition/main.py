...
class FaceRecognitionModel:
    ...
    def predict_label(self, model_input: np.ndarray) -> tuple[str, float]:
        ...
        return self.__class_labels[class_id], predicted_prob[0][class_id]
    ...
    def test_webcam(self):
        ...
        while not finished:
            ...
            for (x, y, w, h) in face_detections:
                ...
                # predict the face-class
                label, probability = self.predict_label(model_input)

                # display class-label
                font = cv2.FONT_HERSHEY_SIMPLEX
                color = (0, probability * 255, (1 - probability) * 255)  # color in BGR
                stroke = 2
                probability_str: str = '%.0f' % (probability * 100)
                cv2.putText(frame, f'{label}', (x, y - 40), font, 1, color, stroke, cv2.LINE_AA)
...
def main():
    ...
    model = FaceRecognitionModel(face_cascade_classifier_path)
